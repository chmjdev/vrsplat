...
Package version stays **0.11.0**: nothing here changes what a consumer resolves.
What changed is that 0.11.0 has been **run** for the first time, and the harness
that runs it had a defect that prevented exactly that.

### Fixed

- **`tools/verify/PackageVerify.cs` had a non-terminating loop.**
  `for (byte tag = 252; tag <= 255; tag++)` can never end — a `byte` cannot
  exceed 255, so the condition is always true and the counter wraps to 0 and
  goes round again. Measured before the fix: the run spun for **22 minutes**,
  logged **12.5 million** failing iterations, grew `verify.log` to **11.4 GB**
  and was still climbing at ~5 MB/s, and never printed a `VERIFY` line. The
  counter is an `int` now, cast to `byte` at the call.

  Worth stating plainly, because it is the second time this repository has been
  bitten by the same shape of thing: the first four iterations **passed** and
  looked exactly like a working test. What followed was 12.5 million lines of
  `TargetInvocationException` from the wrapped-around tags, which reads as "the
  quaternion port is broken" and is nothing of the kind.

### Verified — this supersedes 0.11.0's "UNVERIFIED" preamble

0.11.0 was written by a session with no working Unity process, and said so.
It has now been compiled and run: `./tools/verify/run.sh` on **Unity
6000.3.22f1 + URP 17.3.0**, **zero compile errors**, **24 checks ALL PASS**,
exit 0.

- The **LOD ladder** compiles in — `m_LodEnabled`, `m_LayerLodDistances` and
  `LayerLodEntry` all present by reflection.
- **`SogFileReader`** compiles in, declares `ReadMeta` / `Unlog` /
  `DequantizePositionAxis` / `DequantizeScale` / `DequantizeSh0` /
  `DequantizeQuat`, and the `ISogTextureDecoder` seam is present.
- **`DequantizeQuat` returns a unit quaternion on all four `maxComp` branches**
  (tags 252–255), |q| = 1.000000 each. That is the first actual execution of the
  port taken from `@playcanvas/splat-transform`'s own source.

**What this does not say:** nothing here rendered a frame. The LOD ladder has
never been run against a real capture, and SOG import still cannot decode WebP
pixels, so no `.sog` file has been read end to end. Both remain UNVERIFIED in
the sense `ROADMAP.md` means it.

### Added

- **`AGENTS.md`** — a briefing for an agent picking up work here: what the
  package is, the rules that bind changes, how to verify, the upstream
  relationship, what is open, and the traps. Written for the public repository,
  so it carries no local paths or internal inventory.
- **`package/Editor/Utils/SogFileReader.cs.meta`** — generated by a real Unity
  Editor during the verify run above, which is exactly the condition 0.11.0 said
  it was waiting for ("Unity generates that on first import in a real Editor;
  writing one by hand risked a GUID collision I have no way to check for").
  GUID `482d34ef1...`, checked unique against every other `.meta` in the package.
  It matters: a `.cs` shipped without its `.meta` gets a fresh GUID in every
  consumer, and any serialized reference to it breaks.

### Changed

- The **"17 commits ahead"** figure in `Readme.md`, `docs/upstream.md` and the
  architecture scan was stale the moment it was written and rots on every
  commit. Replaced with the fact that actually matters — **0 behind**, nothing
  upstream unmerged — with the ahead-count given as a dated observation to
  re-count rather than quote.

## 0.11.0 — 2026-09-09

> **Superseded 2026-09-10:** this release has since been compiled and run —
> zero compile errors, 24 checks ALL PASS. See the entry above. The paragraph
> below stands as what was true when it was written.
