...
: "${VRFLATSCORE_REMOTE_HOST:?set VRFLATSCORE_REMOTE_HOST=user@gpu-box (see README.md)}"
REMOTE_WORK="${VRFLATSCORE_REMOTE_WORKDIR:-\$HOME/vrflatscore-work}"
SSH_OPTS=()
[ -n "${VRFLATSCORE_REMOTE_KEY:-}" ] && SSH_OPTS=(-i "$VRFLATSCORE_REMOTE_KEY")
...
ssh "${SSH_OPTS[@]}" "$VRFLATSCORE_REMOTE_HOST" "mkdir -p $REMOTE_WORK/$SCENE_ID/frames"
rsync -az --info=progress2 ${VRFLATSCORE_REMOTE_KEY:+-e "ssh -i $VRFLATSCORE_REMOTE_KEY"} \
  "$FRAMES"/ "$VRFLATSCORE_REMOTE_HOST:$REMOTE_WORK/$SCENE_ID/frames/"

echo "=== 2/4  COLMAP poses (required: LichtFeld trains from a COLMAP dataset) ==="
ssh "${SSH_OPTS[@]}" "$VRFLATSCORE_REMOTE_HOST" bash -s <<REMOTE
set -euo pipefail
...
echo "    $TRAIN_RESOLVED"
ssh "${SSH_OPTS[@]}" "$VRFLATSCORE_REMOTE_HOST" \
...
mkdir -p "$DEST"
rsync -az ${VRFLATSCORE_REMOTE_KEY:+-e "ssh -i $VRFLATSCORE_REMOTE_KEY"} \
  "$VRFLATSCORE_REMOTE_HOST:$REMOTE_WORK/$SCENE_ID/out/"*.ply "$DEST/room.ply"
